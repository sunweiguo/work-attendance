        ...
        <div class="modal-dialog">
            <div class="modal-content">
                ...
                <div class="modal-body">
                        ...
                        <input type="text" name="email" id="username_show_input">
                </div>
                <div class="modal-body">
                    <label class="col-sm-2 control-label" style="color: #00a7d0">邮箱</label>
                    <input type="text" name="email" id="email_show_input">
                </div>
                <div class="modal-body">
                    <label class="col-sm-2 control-label" style="color: #00a7d0">手机号码</label>
                    <input type="text" name="email" id="phone_show_input">
                </div>
                <div class="modal-body">
                    <label class="col-sm-2 control-label" style="color: #00a7d0">性别</label>
                        <label class="radio-inline" >
                            <input type="radio" name="gender" id="gender_show_input1" value="1"> 男
                        </label>
                        <label class="radio-inline">
                            <input type="radio" name="gender" id="gender_show_input2" value="2"> 女
                        </label>
                </div>
    ...
    <div class="modal fade" id="update" tabindex="-1" role="dialog" aria-labelledby="myModalLabel2" aria-hidden="true">
        <div class="modal-dialog">
            <div class="modal-content">
                ...
                <div class="modal-body">
                    <label class="col-sm-2 control-label" style="color: #00a7d0">用户名</label>
                    <input type="text" name="username" id="username_update_input">
                </div>
                <div class="modal-body">
                    <label class="col-sm-2 control-label" style="color: #00a7d0">电话号码</label>
                    <input type="text" name="phoneNumber" id="phone_update_input">
                </div>
                <div class="modal-body">
                    <label class="col-sm-2 control-label" style="color: #00a7d0">性别</label>
                    <label class="radio-inline" >
                        <input type="radio" name="gender" id="gender_update_input1" value="1"> 男
                    </label>
                    <label class="radio-inline">
                        <input type="radio" name="gender" id="gender_update_input2" value="2"> 女
                    </label>
                </div>
                <div class="modal-footer">
...
<script>
    ...
    function showAllAttends(data) {
        window.location.href="/attend/ShowAllAttendsByUserId?userId="+data
    }
    function del(data) {
        alert("删除操作");
    }
    function update(data) {
        $.ajax({
            ...
            success:function (data) {
                ...
                $("#phone_update_input").val(user.phonenumber);
                $("#idHidden").val(user.id);
                if(user.sex==1){
                    var el = document.getElementById('gender_update_input1')
                    el.checked = true;
                }else if(user.sex==2){
                    var el = document.getElementById('gender_update_input2')
                    el.checked = true;
                }
            }
        });
    ...
    function detail(data) {
        $.ajax({
            ...
            success:function (data) {
                var user = JSON.parse(data).data;
                $("#username_show_input").val(user.username);
                $("#phone_show_input").val(user.phonenumber);
                $("#email_show_input").val(user.email);
                if(user.sex==1){
                    var el = document.getElementById('gender_show_input1')
                    el.checked = true;
                }else if(user.sex==2){
                    var el = document.getElementById('gender_show_input2')
                    el.checked = true;
                }
            }
        });
            ...
            .on('error.dt', function (e, settings, techNote, message) {
                ...
                columns: [
                    ...
                    },
                    {
                        "data": "操作",
                        render: function (data, type, full) {
                            return ('<menu type="menu" class="btn btn-info" data-toggle="modal" data-target="#detail" onclick="detail(\'' + full.id + '\')">详情</menu> ' +
                            '<menu type="menu" class="btn btn-warning"  data-toggle="modal" data-target="#update" onclick="update(\'' + full.id + '\')">修改</menu> ' +
                            '<menu type="menu" class="btn btn-danger" onclick="del(\'' + full.id + '\')">删除</menu> '+
                            '<menu type="menu" class="btn btn-yahoo"  onclick="showAllAttends(\'' + full.id + '\')">打卡记录</menu> ');
                        }
                    }
                ]
            })
    ...
    $("#emp_update_btn").click(function(){
        ...
        var phoneNumber = $("#phone_update_input").val();
        var userId = $("#idHidden").val();
        var sex = '';
        var e1 = document.getElementById('gender_update_input1');
        var e2 = document.getElementById('gender_update_input2');
        if(e1.checked==true){
            sex = 1;
        ...
        $.ajax({
            type:"POST",
            url:"/user/updateUser",
            data:{"username":username,"phoneNumber":phoneNumber,"userId":userId,"sex":sex},
            success:function (data) {
